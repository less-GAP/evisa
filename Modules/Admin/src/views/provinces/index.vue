...
const tableConfig = {
  api: (params) => Api.get('provinces/list', {params}),
  addAction: (reload) => {
    router.replace('/provinces/0')
  },
  itemActions: [
    {
      label: '',
      ...
      key: 'edit',
      action(item, reload) {
        router.replace('/provinces/' + item.id)
      }
    },
    {
      label: '',
      ...
      key: 'delete',
      action(item, reload) {
        Api.delete('provinces/' + item.id).then(rs => {
  ...
  columns: [
    {title: 'Mã tỉnh/thành phố', key: 'code'},
    {title: 'Tên tỉnh/thành phố', key: 'name'},
  ...
  selectionActions: [
    {
      title: 'Hoạt động',
      action(selectedKeys) {
        return Api.post('provinces/activeList', {
          'items': selectedKeys,
          'status': 'A'
        }).then(rs => {
          notification[rs.data.code == 0 ? 'error' : 'success']({
