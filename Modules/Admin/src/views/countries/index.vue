...
const tableConfig = {
  ...
  itemActions: [
    ...
    },
    {
      ...
      key: 'delete',
      action(item, reload) {
        Api.delete('countries/' + item.id).then(rs => {
  ...
  columns: [
    {title: 'Mã quốc gia', key: 'code'},
    {title: 'Tên quốc gia', key: 'name'},
    {title: 'Mã A3', key: 'code_A3'},
    {title: 'Mã N3', key: 'code_N3'},
    {title: 'Vùng', key: 'region'},
  ...
  selectionActions: [
    {
      title: 'Hoạt động',
      action(selectedKeys) {
        return Api.post('countries/activeList', {
          'items': selectedKeys,
          'status': 'A'
        }).then(rs => {
          notification[rs.data.code == 0 ? 'error' : 'success']({
