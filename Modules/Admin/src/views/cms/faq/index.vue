...
<template>
  ...
  <DataTable @order="(item,index)=>updateApi(item.id,{order:index})" @register="registerTable" v-bind="tableConfig">
    ...
    <template #cell[title]="{item,column}">
      <a class="cursor-pointer text-blue-100" :href="$url('post/'+item.slug)" target="_blank"
      >{{ item.title }}</a>
    </template>
