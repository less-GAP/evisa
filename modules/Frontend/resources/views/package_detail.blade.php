...
<div class="main" id="site-main">
    @if(!empty($package))
        @if($package['position'] == 'right')
            <section id="product-package-detail" class="product_packages" style="background: {{$package['background']}}">
                                ...
                                @if(!empty($package['courses']))
                                    <div class="flex items-center justify-center package-course-control mt-5">
                                        @foreach($package['courses'] as $key => $cur)
                                            ...
                                            @if($key > 0)
                                                <div
                                                    class="flex items-center px-1 mx-3 rounded-full w-14 h-7 btn-handleToggle cursor-pointer active" data-current="4" id="handleToggle">
                ...
                <section id="package-includes-{{$key}}" class="package_includes @if($key > 0) hidden @endif">
                    <div class="container relative py-12 lg:py-16 2xl:py-20">
                        @if(!empty($cur['products']))
                            ...
                            <div id="product-tab" class="mt-16 text-sm product-info md:text-xl product-tab">
                                @foreach($cur['products'] as $k => $pro)
    ...
    @if(!empty($other_packages))
        <section id="package-other">
            <div class="container relative py-12 lg:py-16 2xl:py-20">
                ...
                <div class="row">
                    @foreach($other_packages as $other)
                        <div class="w-full md:w-1/3 col">
                            <div class="product rounded-2xl md:rounded-3xl xl:rounded-[50px] p-5 lg:p-12">
                                <div class="mb-4 text-center img">
                                    <a href="{{$other['slug']}}">
                                        <img src="{{$other['image_url']}}" alt="" class="block mx-auto h-auto md:h-[200px] object-contain w-auto">
                                    </a>
                                </div>
                                <h4 class="product-name"><a href="{{$other['slug']}}">
                                        {{$other['package_name']}}: {{$other['name']}} - {{$other['time']}}
                                    </a>
                                </h4>
                            </div>
                        </div>
    ...
    {{--        </script>--}}
    <x-splade-script>
        if (document.getElementById('product-package-detail')) {
        var swiper = new Swiper('#product-package-detail .swiper', {
        slidesPerView: 1,
        spaceBetween: 30,
        loop: true,
        simulateTouch: false,
        });
        let btn = document.getElementById('handleToggle');
        btn.addEventListener('click', () => {
        let current = btn.getAttribute('data-current');
        if (current == 7) {
        btn.setAttribute('data-current', 4);
        ...
        btn.setAttribute('data-current', 7);
